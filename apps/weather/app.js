  ...
  try {
    ...
    try {
      ...
      elements.statusError.innerHTML = `
        ...
        <button id="retry-weather-btn" style="background: var(--theme-accent); color:#0f172a; border:none; padding: 0.6rem 1.2rem; border-radius:12px; cursor:pointer; font-weight:600; margin-top: 10px;">Reintentar</button>
      `;
      // El botón acaba de crearse, así que se engancha aquí.
      elements.statusError
        .querySelector('#retry-weather-btn')
        ?.addEventListener('click', retryWeatherFetch);
      safeCreateIcons();
    }
  }
}
...
// Se engancha por id tras pintar el error: como app.js es un módulo, un
// `onclick` incrustado no encontraría la función (nada es global).
